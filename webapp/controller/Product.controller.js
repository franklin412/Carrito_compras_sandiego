sap.ui.define([
	...
	BaseController, formatter, MessageToast, MessageBox, Filter, FilterOperator) {
	"use strict";
	...
	return BaseController.extend("zsandiego.carritocompras.controller.Product", {
		...
		fnUpdateProduct: function (productId, productType, categoryId) {
            var sPath = "catalogo>/" + this.getModel("catalogo").createKey("materialesSet", {
                Item: productId,
                Catyp: productType,
                Catnr: categoryId
            });            
		...
		_checkIfProductAvailable: function (sPath) {
			...
			var oData = oModel.getData(sPath.replace("catalogo>", ""));
		...
		onToggleCart: function (oEvent) {
			...
			var oEntry = this.getView().getBindingContext("catalogo").getObject();
            this._setLayout(bPressed ? "Three" : "Two");
			this.getRouter().navTo(bPressed ? "productCart" : "product", {
                id: oEntry.Catnr,
                productType: oEntry.Catyp,
				productId: oEntry.Item
			});
		},
